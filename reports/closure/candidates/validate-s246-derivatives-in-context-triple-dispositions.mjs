...
const candidatePath = path.join(here, "S246_DERIVATIVES_IN_CONTEXT_TRIPLE_DISPOSITIONS.jsonl");
...
const coursePath = path.join(root, "content/courses/derivatives-in-context/course.json");

const expectedDistribution = {
  decisions: { KEEP: 0, REVISE: 11, ESCALATE: 0 },
  visualDecisions: { REQUIRED: 7, PREFERRED: 2, SUFFICIENT: 2, ESCALATE: 0 },
  gradeLanguageDecisions: { FIT: 7, REVISE: 4, ESCALATE: 0 }
};
...
const evidenceFile = (reference) => {
  ...
  const prefixes = ["content/", "reports/", "PREMIUM_PENDING_WORKLOAD_QUEUE.csv"];
...
for (const lessonId of expectedIds) {
  ...
  if (record.recordId !== `S246-DC-${lessonId}`) errors.push(`${lessonId}: recordId does not follow the sealed course pattern`);
  ...
  if (record.reviewer !== "ChatGPT Work independent assessor (derivatives-in-context)") errors.push(`${lessonId}: unexpected reviewer`);
  ...
  if (!Array.isArray(record.evidenceRefs) || record.evidenceRefs.length < 3 || record.evidenceRefs.some((ref) => !String(ref).trim())) {
    ...
    const lessonRef = `content/courses/derivatives-in-context/lessons/${lessonId}.json`;
    ...
    if (!record.evidenceRefs.some((ref) => /VIS01_PLACEMENTS|PREMIUM_PENDING_WORKLOAD_QUEUE/.test(String(ref)))) errors.push(`${lessonId}: missing visual or queue evidence`);
  ...
  if (!fs.existsSync(lessonSource)) errors.push(`${lessonId}: missing source ${card.source}`);
    ...
    if (!Array.isArray(lesson.steps) || !lesson.steps.length || !Array.isArray(lesson.remedials) || !lesson.remedials.length) errors.push(`${lessonId}: full lesson/remedial path missing`);
