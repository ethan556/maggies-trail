...
const packetPath = 'reports/closure/candidates/S247_CCSS_HSF_DOMAIN_SCOPE_BATCH05.jsonl';
...
const batch04PacketSha256 = hashFile('reports/closure/candidates/S247_CCSS_HSF_DOMAIN_SCOPE_BATCH04.jsonl');
const expectedCourseCounts = { 'trig-graphs-inverses': 7, 'curve-analysis': 16, 'derivative-rules': 17 };
const expectedRouting = {
  'trig-graphs-inverses': ['HSF.TF', 'HSF.IF'],
  'curve-analysis': ['HSF.IF'],
  'derivative-rules': ['HSF.IF']
};
...
if (openHsf.length !== 89) fail(`Expected 89 post-batch-04 open HSF edges, found ${openHsf.length}`);
if (JSON.stringify(records.map((record) => record.edgeId)) !== JSON.stringify(openHsf.slice(0, 40).map((dossier) => dossier.edgeId))) {
  fail('Batch 05 is not the next 40 current CCSS-MATH|HSF candidates');
}
if (records.some((record) => globallyDecided.has(record.edgeId))) fail('Batch 05 overlaps the shared decision ledger');
...
for (const record of records) {
  ...
  if (record.deltaFromBatch04.priorIntegratedRejectedEdges !== 160
    || record.deltaFromBatch04.priorPacketSha256 !== batch04PacketSha256
    || record.deltaFromBatch04.remainingHsfCandidatesBeforeBatch05 !== 89) fail(`Delta baseline drift ${record.edgeId}`);
...
const sample = [...records]
  .sort((a, b) => sha256(`${currentHashes.evidenceDossiersSha256}|batch05|${a.edgeId}`)
    .localeCompare(sha256(`${currentHashes.evidenceDossiersSha256}|batch05|${b.edgeId}`)))
  .slice(0, 8);
