...
const candidatePath = path.join(here, "S246_MULTIPLY_BIGGER_TRIPLE_DISPOSITIONS.jsonl");
...
const coursePath = path.join(root, "content/courses/multiply-bigger/course.json");

const expectedDistribution = {
  decisions: { KEEP: 4, REVISE: 10, ESCALATE: 0 },
  visualDecisions: { REQUIRED: 6, PREFERRED: 2, SUFFICIENT: 6, ESCALATE: 0 },
  gradeLanguageDecisions: { FIT: 10, REVISE: 4, ESCALATE: 0 }
};
...
const evidenceFile = (reference) => {
  ...
  const candidates = [
    "content/", "reports/", "PREMIUM_PENDING_WORKLOAD_QUEUE.csv"
  ];
...
for (const lessonId of expectedIds) {
  ...
  if (record.recordId !== `S246-MB-${lessonId}`) errors.push(`${lessonId}: recordId does not follow the sealed course pattern`);
  ...
  if (record.reviewer !== "ChatGPT Work independent assessor (multiply-bigger)") errors.push(`${lessonId}: unexpected reviewer`);
  if (!Number.isFinite(Date.parse(record.reviewedAt))) errors.push(`${lessonId}: invalid reviewedAt`);
  if (String(record.rationale).trim().length < 180) errors.push(`${lessonId}: rationale is not substantive`);
  if (String(record.reopenCondition).trim().length < 140) errors.push(`${lessonId}: reopenCondition is not substantive`);
  ...
  if (!Array.isArray(record.evidenceRefs) || record.evidenceRefs.length < 3 || record.evidenceRefs.some((ref) => !String(ref).trim())) {
    ...
    const lessonRef = `content/courses/multiply-bigger/lessons/${lessonId}.json`;
    ...
    if (!record.evidenceRefs.some((ref) => /VIS01_PLACEMENTS|VIS03_FIGURE_EXEMPLAR_DRIFT|PREMIUM_PENDING_WORKLOAD_QUEUE/.test(String(ref)))) {
      errors.push(`${lessonId}: missing visual or queue evidence`);
    }
