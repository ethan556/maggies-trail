...
const packetPath = 'reports/closure/candidates/S247_CCSS_HSF_DOMAIN_SCOPE_BATCH04.jsonl';
...
const batch03PacketSha256 = hashFile('reports/closure/candidates/S247_CCSS_HSF_DOMAIN_SCOPE_BATCH03.jsonl');
const expectedCourseCounts = { 'function-analysis': 2, 'limits-continuity': 15, 'polar-parametric': 15, 'trig-graphs-inverses': 8 };
const expectedRouting = {
  'function-analysis': ['HSF.IF', 'HSF.BF'],
  'limits-continuity': ['HSF.IF'],
  'polar-parametric': ['HSF.TF', 'HSG.GPE'],
  'trig-graphs-inverses': ['HSF.TF', 'HSF.IF']
};
...
if (records.length !== 40 || new Set(records.map((record) => record.edgeId)).size !== 40) fail('Packet must contain 40 unique records');
if (openHsf.length !== 129) fail(`Expected 129 post-batch-03 open HSF edges, found ${openHsf.length}`);
if (JSON.stringify(records.map((record) => record.edgeId)) !== JSON.stringify(openHsf.slice(0, 40).map((dossier) => dossier.edgeId))) {
  fail('Batch 04 is not the next 40 current CCSS-MATH|HSF candidates');
}
if (records.some((record) => globallyDecided.has(record.edgeId))) fail('Batch 04 overlaps the shared decision ledger');
...
for (const record of records) {
  ...
  if (record.deltaFromBatch03.priorIntegratedRejectedEdges !== 120
    || record.deltaFromBatch03.priorPacketSha256 !== batch03PacketSha256
    || record.deltaFromBatch03.remainingHsfCandidatesBeforeBatch04 !== 129) fail(`Delta baseline drift ${record.edgeId}`);
...
const sample = [...records]
  .sort((a, b) => sha256(`${currentHashes.evidenceDossiersSha256}|batch04|${a.edgeId}`)
    .localeCompare(sha256(`${currentHashes.evidenceDossiersSha256}|batch04|${b.edgeId}`)))
  .slice(0, 8);
