...
- **The live backlog is 5,288, 9,159 below the 14,447-row reference.** The authoritative CSV passes uniqueness, completeness and priority-domain checks and is sealed by SHA-256 `173266b00a6faf5617946023ba2a2cd08a225104b258249ac0316c715a57206d` at commit `3793c45c`.
- **The queue is using the wrong unit of work.** Every row can be assigned exactly once to 146 primary portfolios: 129 course portfolios, 1 standards parent-family portfolio, 14 generator domains and 2 shared programme/engine portfolios. The 2 exact standards codes and 57 exact generator tags remain required subgroups. That is 36.22× fewer context scopes without deleting or auto-closing a single task.
...
The source is `PREMIUM_PENDING_WORKLOAD_QUEUE.csv`: one row per currently open closure obligation. The compiler verifies 5288/5288 unique work IDs, 0 missing required records, and 0 invalid priority values. The portfolio CSV is a derived execution view; the queue remains the source of truth.

## The breakthrough: 5,288 rows become 146 claimable portfolios

| Portfolio class | Queue rows | Primary scopes | Rows per scope | P0 rows | Maximum scope |
|---|---:|---:|---:|---:|---:|
| COURSE_PORTFOLIO | 5,086 | 129 | 39.43 | 204 | 70 |
...
The 129 course portfolios cover 5,086 source-local rows. A course assessor reads the full lesson set once and emits lesson/visual/language dispositions, progression and choice jobs, math and figure requirements, revision contracts and standards evidence summaries. Implementers receive only exact owned files and deltas.

Top closure-leverage course portfolios:

| Course | Rows | P0 | Lessons | Workstreams |
|---|---:|---:|---:|---|
| number-system | 70 | 12 | 16 | CHOICE_SURFACE_INTEGRITY, GRADE_LANGUAGE_REVIEW, ILLUSTRATION_REPLACEMENT, LESSON_COMPLETE_DISPOSITION, LESSON_PROGRESSION_AND_DUPLICATION, QUESTION_DIVERSITY_AND_TRANSFER, VISUAL_FIRST_REPRESENTATION |
| fractions-add | 64 | 14 | 14 | CHOICE_SURFACE_INTEGRITY, GRADE_LANGUAGE_REVIEW, ILLUSTRATION_REPLACEMENT, LESSON_COMPLETE_DISPOSITION, LESSON_PROGRESSION_AND_DUPLICATION, QUESTION_DIVERSITY_AND_TRANSFER, VISUAL_FIRST_REPRESENTATION |
| proportional-relationships | 64 | 9 | 16 | CHOICE_SURFACE_INTEGRITY, GRADE_LANGUAGE_REVIEW, ILLUSTRATION_REPLACEMENT, LESSON_COMPLETE_DISPOSITION, LESSON_PROGRESSION_AND_DUPLICATION, VISUAL_FIRST_REPRESENTATION |
...
| expressions-equations | 62 | 5 | 18 | GRADE_LANGUAGE_REVIEW, ILLUSTRATION_REPLACEMENT, LESSON_COMPLETE_DISPOSITION, LESSON_PROGRESSION_AND_DUPLICATION, VISUAL_FIRST_REPRESENTATION |
| circle-theorems | 59 | 0 | 16 | CHOICE_SURFACE_INTEGRITY, GRADE_LANGUAGE_REVIEW, LESSON_COMPLETE_DISPOSITION, LESSON_PROGRESSION_AND_DUPLICATION, VISUAL_FIRST_REPRESENTATION |
...
| right-triangles-trig | 57 | 5 | 15 | CHOICE_SURFACE_INTEGRITY, GRADE_LANGUAGE_REVIEW, ILLUSTRATION_REPLACEMENT, LESSON_COMPLETE_DISPOSITION, VISUAL_FIRST_REPRESENTATION |
...
- **Illustrations:** 156 rows compile into 100 figure+course contracts and 100 write batches capped at 20 placements. The 0 live `count-on-hops` placements require typed semantic figure specifications, not bespoke pictures.
...
- **Progression and choices:** 288 progression rows are 59 course contracts; 447 choice rows are 131 authored-course or generator contracts.
...
2. **Assessment cohort.** Review 3 courses concurrently with four active agents, or 6 with eight active agents. One assessor owns one course. The 110 remaining course reviews become 37 three-assessor cohorts or 19 six-assessor cohorts.
...
All 337 current signed lesson reviews resolved 1,011 generic disposition rows and left 254 implementation or escalation rows. If the same 75.4% revision-required rate held—a planning scenario, not a forecast—the remaining 1,364 reviews would resolve 4,092 generic rows, create about 1,028 implementation rows and reduce the queue by about 3,064 net rows before those fixes are completed.
...
5. Implement or adjudicate the 254 current revision/escalation packets before reviewing their courses again.
...
- Does the revision-required rate remain near the observed 75.4% once less risky courses are reviewed? Recalculate after every 100 decisions.
...
- The disposition scenario is based on all 337 current reviewed lessons and is deliberately labeled as a scenario.
