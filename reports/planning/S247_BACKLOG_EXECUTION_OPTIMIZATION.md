...
- **The live backlog is 5,284, 9,163 below the 14,447-row reference.** The authoritative CSV passes uniqueness, completeness and priority-domain checks and is sealed by SHA-256 `1103afc60f994dabd923192bba243bbbf36b5bccaaa8d0f641298a01152b6931` at commit `468df043`.
- **The queue is using the wrong unit of work.** Every row can be assigned exactly once to 146 primary portfolios: 129 course portfolios, 1 standards parent-family portfolio, 14 generator domains and 2 shared programme/engine portfolios. The 2 exact standards codes and 57 exact generator tags remain required subgroups. That is 36.19× fewer context scopes without deleting or auto-closing a single task.
...
The source is `PREMIUM_PENDING_WORKLOAD_QUEUE.csv`: one row per currently open closure obligation. The compiler verifies 5284/5284 unique work IDs, 0 missing required records, and 0 invalid priority values. The portfolio CSV is a derived execution view; the queue remains the source of truth.

## The breakthrough: 5,284 rows become 146 claimable portfolios

| Portfolio class | Queue rows | Primary scopes | Rows per scope | P0 rows | Maximum scope |
|---|---:|---:|---:|---:|---:|
| COURSE_PORTFOLIO | 5,082 | 129 | 39.4 | 118 | 63 |
...
The 129 course portfolios cover 5,082 source-local rows. A course assessor reads the full lesson set once and emits lesson/visual/language dispositions, progression and choice jobs, math and figure requirements, revision contracts and standards evidence summaries. Implementers receive only exact owned files and deltas.
...
All 290 current signed lesson reviews resolved 870 generic disposition rows and left 213 implementation or escalation rows. If the same 73.4% revision-required rate held—a planning scenario, not a forecast—the remaining 1,411 reviews would resolve 4,233 generic rows, create about 1,036 implementation rows and reduce the queue by about 3,197 net rows before those fixes are completed.
...
5. Implement or adjudicate the 213 current revision/escalation packets before reviewing their courses again.
...
- Does the revision-required rate remain near the observed 73.4% once less risky courses are reviewed? Recalculate after every 100 decisions.
...
- The disposition scenario is based on all 290 current reviewed lessons and is deliberately labeled as a scenario.
