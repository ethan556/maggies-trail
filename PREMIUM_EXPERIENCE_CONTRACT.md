...
| 7 | **Excellent at 390px** — no overflow, no sub-44px targets, no wrapped one-row structures. **One ratified exception (S242, 2026-08-15): E5-EX1** — a dense cell grid on a stage too narrow for the full pitch may fall to ≥ 38px *provided no target overlaps*. Applies only to aspect-ratio-sized grid cells under a capped track (plotPoint at `cols: 8`, ≈390px); 44px is unreachable there by arithmetic (8 × 44 = 352 > ~334px of stage). Overlap is never waived, and nothing else is exempt. See `GRAPH_FIGURE_STANDARD.md` E5-EX1. | machine (e2e) + pixels |
