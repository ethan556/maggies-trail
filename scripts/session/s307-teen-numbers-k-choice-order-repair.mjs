...
const lessonsDirectory = join(process.cwd(), "content", "courses", "teen-numbers-k", "lessons");
const canonicalIds = ["o0", "o1", "o2", "o3"];
const expectedInventoryHash = "7f629fc6f4854f616c4f473800aadf94ba2862edc902f288221757f4e7596d6f";

function fail(message) { throw new Error(`S307 teen-numbers-k choice-order repair: ${message}`); }
...
if (targets.length !== 26 || createHash("sha256").update(inventory).digest("hex") !== expectedInventoryHash)
...
console.log(`S307 teen-numbers-k choice-order repair: ${changed.length ? `updated ${changed.join(", ")}` : "already current"}`);
