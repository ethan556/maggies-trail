...
const lessonsDirectory = join(process.cwd(), "content", "courses", "counting-to-20-k", "lessons");
const canonicalIds = ["a", "b", "c", "d"];
const expectedInventoryHash = "b07f754430073a77bc95efb9b51044190bf4cbf234166ba083a7540f61f5fe53";

function fail(message) { throw new Error(`S310 counting-to-20-k choice-order repair: ${message}`); }
...
if (records.length !== 13) fail(`expected 13 lessons, found ${records.length}`);
...
if (targets.length !== 15 || createHash("sha256").update(inventory).digest("hex") !== expectedInventoryHash)
...
console.log(`S310 counting-to-20-k choice-order repair: ${changed.length ? `updated ${changed.join(", ")}` : "already current"}`);
