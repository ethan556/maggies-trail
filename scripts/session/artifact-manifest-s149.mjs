...
const output='SESSION149_ARTIFACTS.json';
...
const manifest={session:149,root:'maggies-trail-session-149',scope:'all packaged regular files except this manifest and explicitly forbidden dependency/build/log paths',excluded:[output,'node_modules/**','.next/**','.cml-build/**','coverage/**','test-results/**','playwright-report/**','.git/**','*.log','*.tsbuildinfo','.DS_Store'],fileCount:files.length,totalBytes:files.reduce((n,x)=>n+x.size,0),files};
