...
const lessonsDirectory = join(process.cwd(), "content", "courses", "compare-numbers-k", "lessons");
const canonicalIds = ["o0", "o1", "o2", "o3"];
const expectedInventoryHash = "39dedc24cddafa535168af10c39e5907bc3c91747d3fe5bbc6146c6496a1e36f";

function fail(message) {
  throw new Error(`S306 compare-numbers-k choice-order repair: ${message}`);
}
...
const targets = records.flatMap((record) => record.lesson.steps
  ...
  .map((step) => ({ lessonId: record.lesson.id, stepId: step.id })));
const inventory = targets.map((target) => `${target.lessonId}/${target.stepId}`).join("\n");
if (targets.length !== 34 || createHash("sha256").update(inventory).digest("hex") !== expectedInventoryHash)
...
console.log(`S306 compare-numbers-k choice-order repair: ${changed.length ? `updated ${changed.join(", ")}` : "already current"}`);
