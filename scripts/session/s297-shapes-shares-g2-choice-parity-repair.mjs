...
const lessonsDirectory = join(process.cwd(), "content", "courses", "shapes-shares-g2", "lessons");
const canonicalIds = ["a", "b", "c"];

function fail(message) {
  throw new Error(`S297 shapes-shares-g2 choice parity repair: ${message}`);
}
...
if (targets.length !== 19) fail(`expected 19 MCQs, found ${targets.length}`);
...
for (const record of records) {
  ...
  writeFileSync(record.path, `${JSON.stringify(record.lesson, null, 2)}\n`);
  changed.push(record.lesson.id);
}

console.log(`S297 shapes-shares-g2 choice parity repair: ${changed.length ? `updated ${changed.join(", ")}` : "already current"}`);
