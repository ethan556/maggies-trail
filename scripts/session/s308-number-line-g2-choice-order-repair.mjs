...
const lessonsDirectory = join(process.cwd(), "content", "courses", "number-line-g2", "lessons");
const canonicalIds = ["o0", "o1", "o2", "o3"];
const expectedInventoryHash = "953a7cced065a02da1e5ec0f628f776d69d5ef09879e3dd475b3ae1a9c9ec2c1";

function fail(message) { throw new Error(`S308 number-line-g2 choice-order repair: ${message}`); }
...
if (targets.length !== 14 || createHash("sha256").update(inventory).digest("hex") !== expectedInventoryHash)
...
console.log(`S308 number-line-g2 choice-order repair: ${changed.length ? `updated ${changed.join(", ")}` : "already current"}`);
