...
if (packet.length !== 40 || new Set(packet.map((record) => record.edgeId)).size !== 40) throw new Error("Batch 03 must contain 40 unique edges.");
...
for (const record of packet) {
  ...
  if (record.deltaFromBatch02?.priorIntegratedRejectedEdges !== 80) throw new Error(`Missing batch-02 lineage: ${record.edgeId}`);
}
...
console.log(JSON.stringify({ mode:apply ? "apply" : "check", priorDecisions:ledger.decisions?.length ?? 0, appendedDecisions:packet.length, nextDecisions:next.decisions.length, packetSha256:sha256(packetText) }, null, 2));
