...
if (!packetArg) throw new Error("Usage: node scripts/standards/apply-remaining-hs-category-scope-packet-s250.mjs <packet.jsonl> [--apply]");

const supported = new Set([
  "HSA", "HSG", "HSN", "HSS",
  "CA-HSA", "CA-HSG", "CA-HSN", "CA-HSS",
  "MA.HS.HSA", "MA.HS.HSG", "MA.HS.HSN", "MA.HS.HSS",
]);
...
if (packet.length === 0 || packet.length > 40 || new Set(packet.map((record) => record.edgeId)).size !== packet.length) {
  throw new Error("High-school category packet must contain 1–40 unique edges.");
}
...
if (!supported.has(candidateCode)) throw new Error(`Unsupported high-school category contract: ${candidateCode}`);
...
for (const record of packet) {
  ...
  if (record.decision !== "rejected" || record.candidateCode !== candidateCode
    || !record.claimBoundary?.startsWith(`This decision rejects only the coarse ${candidateCode} portfolio locator.`)) {
...
console.log(JSON.stringify({ mode: apply ? "apply" : "check", candidateCode, packet: path.relative(root, packetPath).replaceAll("\\", "/"), priorDecisions: ledger.decisions?.length ?? 0, appendedDecisions: packet.length, nextDecisions: next.decisions.length, packetSha256: sha(packetText) }, null, 2));
