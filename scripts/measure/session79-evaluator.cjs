...
for(const [gen,form] of forms)for(const band of ['support','core','stretch'])for(let i=0;i<120;i++){const v=V.variantForGenForm(gen,form,`session79-eval:${gen}:${form}:${band}:${i}`,band),w=v.widget;builds++;
