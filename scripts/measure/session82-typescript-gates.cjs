...
const tempTypes=path.join(root,'src/lib/.session82-types.d.ts');
const tempVariants=path.join(root,'src/lib/.session82-variants-semantic.ts');
let variants=fs.readFileSync(path.join(root,'src/lib/variants.ts'),'utf8');
variants=variants.replace('import type { TWidget } from "./schema";','import type { TWidget, Band } from "./.session82-types";');
