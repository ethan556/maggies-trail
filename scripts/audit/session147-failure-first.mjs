...
const report={session:147,purpose:'Failure-first regression audit before Session 147 release.',checks,summary:{passed:checks.length-failures.length,failed:failures.length,total:checks.length},packageJsonSha256:packageJsonHash,packageLockSha256:lockHash,passed:failures.length===0};
writeFileSync(join(root,'SESSION147_FAILURE_FIRST_AUDIT.json'),JSON.stringify(report,null,2)+'\n');
writeFileSync(join(root,'SESSION147_FAILURE_FIRST_AUDIT.md'),['# Session 147 failure-first audit','',`- Result: **${report.passed?'PASS':'FAIL'}**`,`- Checks: **${report.summary.passed}/${report.summary.total}**`,'','## Checks','',...checks.map(c=>`- ${c.passed?'PASS':'FAIL'} — \`${c.id}\`: ${c.detail}`),''].join('\n'));
if(failures.length){for(const f of failures)console.error(`${f.id}: ${f.detail}`);process.exit(1)}
console.log(`Session 147 failure-first audit passed: ${checks.length}/${checks.length}`);
