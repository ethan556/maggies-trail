...
for lesson_id,rel in TARGETS.items():
 old=json.loads((BASE/f'{lesson_id}.json').read_text(encoding='utf-8')); new=json.loads((ROOT/rel).read_text(encoding='utf-8'))
...
for rel,h in ledger.items():
 ...
 if hashlib.sha256((ROOT/rel).read_bytes().replace(b'\r\n',b'\n')).hexdigest()!=h: raise AssertionError(f'non-target lesson drift: {rel}')
 unchanged+=1
