...
export const SAMPLES: unknown[] = [
  ...
  /* S238 growth models (NOT-POSSIBLE rows closed): the same {x, moves} state as the ladder,
     with x the radius and every readout an exact π-multiple. */
  { type:"relatedRatesLab", model:"circleArea", prompt:"Grow the disc to radius 5 with dr/dt = 1 and watch dA/dt climb.", ladderLength:5, horizontalRate:1, targetX:5, startX:1, requiredMoves:3, successFeedback:"At r = 5 the area runs at 10π per second while the radius still grows at 1.", explorationFeedback:"Pause at a few radii and read dA/dt at each one before checking.", positionFeedback:"Take the radius all the way out to 5 and read the rate there." },
  { type:"relatedRatesLab", model:"sphereVolume", prompt:"Inflate the balloon to radius 4 with dr/dt = 1 and watch dV/dt outrun the disc.", ladderLength:5, horizontalRate:1, targetX:4, startX:1, requiredMoves:3, successFeedback:"At r = 4 the volume runs at 64π per second — the square in 4πr² is doing that.", explorationFeedback:"Stop at a few radii and compare dV/dt against the disc's 2πr.", positionFeedback:"Inflate the balloon out to radius 4 and read the rate there." },
  { type:"quadraticExplore", prompt:"Bring a down from 3 to 1/3 and open the parabola wider than the dashed parent.", form:"vertex", aDen:3, aMin:1, aMax:9, aStart:9, targetA:1, hMin:0, hMax:0, hStart:0, targetH:0, kMin:0, kMax:0, kStart:0, targetK:0, gridMax:7, showParent:true, successFeedback:"a = 1/3 hugs the axis — wider than the parent because every height shrank to a third.", shapeFeedback:"Wider means sitting BELOW the dashed parent away from the vertex — keep lowering a.", vertexFeedback:"The vertex is pinned at the origin here — only the stretch moves." },
