...
const mcq=(r:Rand,tag:string,prompt:string,correct:[string,string],wrongOrFirst:Array<[string,string]>|[string,string],...moreWrong:Array<[string,string]>):Variant=>{const wrong:Array<[string,string]>=moreWrong.length?[wrongOrFirst as [string,string],...moreWrong]:wrongOrFirst as Array<[string,string]>;const seen=new Set<string>();const all=[correct,...wrong].filter(([l])=>!seen.has(l)&&seen.add(l));/* S242 / GEN-03. NO FILLER OPTIONS. This padded every short list up to four with a literal option
   labelled `Alternative 3` — 30 of them reach learners, e.g. "Solve 2^x = 16" offering 4, 8, 2 and
   `Alternative 3`. That is not a distractor: rule 3 wants a COMPUTED REAL MISCONCEPTION, and rule 7
   says refusing to manufacture one is a success. A three-option item is a fine item; a four-option
   item with a giveaway is not. */
  const options=shuffle(r,all.slice(0,4).map(([label,feedback],i)=>({label,feedback:padFeedback(feedback),correct:i===0}))).map((o,i)=>({id:`o${i}`,...o}));return{tag,answer:options.find(o=>o.correct)!.id,widget:{type:'mcq',prompt,options}}};
...
function systems(r:Rand,b:Band,form:string):Variant{
 ...
 if(concept==='classify-systems'){
  const kind=choose(r,['one solution','no solution','infinitely many solutions'] as const),m1=pick(r,-4,4)||2,b1=pick(r,-6,6),m2=kind==='one solution'?(m1===4?m1-1:m1+1):m1,b2=kind==='infinitely many solutions'?b1:b1+pick(r,1,5);
  const CLASSIFY_WRONG:Record<string,[string,string]>={'one solution':['Exactly one solution point.','Check whether the slopes differ or the equations are actually the same line.'],'no solution':['No point satisfies both lines at once.','Distinct parallel lines are required for a system with no solution.'],'infinitely many solutions':['Every point on the line satisfies both equations.','Infinitely many solutions requires the exact same line.']};
 ...
 if(concept==='solve-by-graphing'||concept==='system-solution'){
  /* S242 / GEN-03. THE FOUR OPTIONS ARE (x,y), (y,x), (-x,y) and (x,-y), so the draw must keep all
   * four distinct or the mcq helper dedups them away. x=0 collapses the x-flip onto the answer, y=0
   * collapses the y-flip, and x=y collapses the swap; at x=y=0 all four become (0, 0) and the item
   * ships with ONE option. That happened once in 450 draws — "Which point solves y = -3x + 0 and
   * y = -2x + 0?" — and was invisible because the mcq helper padded the list back to four with
   * filler labelled `Alternative 3`. Removing the filler is what surfaced it. This is the trap
   * collision CLAUDE.md calls the #1 failure mode, and the remedy it prescribes is to guard the
   * DRAW rather than to widen anything: the resample is bounded, and the pool loses ~24 of 154
   * (x, y) pairs, which the freshness gate still clears. */
  let x=pick(r,-4,6),y=pick(r,-5,8);
  for(let guard=0;guard<24&&(x===0||y===0||x===y);guard++){x=pick(r,-4,6);y=pick(r,-5,8)}
  if(x===0)x=3; if(y===0)y=5; if(x===y)y=x+1;
  const m1=pick(r,-4,4)||2,m2=m1===4?m1-1:m1+1,b1=y-m1*x,b2=y-m2*x;
