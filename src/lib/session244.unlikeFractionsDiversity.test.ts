...
const DIR = join(__dirname, "../../content/courses/unlike-fractions-g5/lessons");
...
function questionJob(prompt: string): "estimate" | "reason" | "regroup" | "context" | "compute" {
  const value = prompt.toLowerCase();
  if (/estimate|benchmark|reasonable|predicts/.test(value)) return "estimate";
  if (/why|which|explain|what does that show/.test(value)) return "reason";
  if (/regroup|trade|improper|mixed number/.test(value)) return "regroup";
  if (/jug|litre|trail|poured|story/.test(value)) return "context";
  return "compute";
}

describe("S244 unlike fractions — repetition and transfer guard", () => {
  it("gives every second manipulation a different visual action", () => {
  ...
  it("never repeats an assessed stem-and-option identity in one sitting", () => {
    for (const lesson of lessons) {
  ...
  it("keeps every challenge distinct and spans the intended question jobs", () => {
    ...
    expect([...familyJobs].sort()).toEqual(["compute", "context", "estimate", "reason", "regroup"]);
  });
