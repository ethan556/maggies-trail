...
const directory = join(process.cwd(), "content", "courses", "counting-120", "lessons");
const lessons = readdirSync(directory)
  .filter((file) => file.endsWith(".json"))
  .sort()
  .map((file) => Lesson.parse(JSON.parse(readFileSync(join(directory, file), "utf8"))));

describe("S298 counting-120 choice parity repair", () => {
  ...
  it("keeps all fifteen Counting to 120 lessons schema-valid", () => {
